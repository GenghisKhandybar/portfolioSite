...
study_time <- read.csv("collegeFirstYear.csv") %>% 
  ...
  mutate(Assignment = ifelse(Title != "", str_trunc(Title, 59), str_c(Type, Tasks_Predicted, Units, sep = " "))) %>% 
...
widget <- DT::datatable(study_time %>% select(Completed,Class,Assignment,Minutes, Min_Estimated), rownames = FALSE)
...
So, does Cal Poly require a full 25-35 hours a week of homework? Not for me at least. But then again, slogan wouldn't be very inspirational if were any less. Also, I'm fairly certain architecture majors exceed 35 hours by at least a factor of two anyway.
